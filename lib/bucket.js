...
let Bucket = class Bucket extends extent_1.DocumentExtent {
    ...
    async connect(cluster, initialize) {
        ...
        common_1.promisifyAll(this.api, 'append', 'counter', 'get', 'getAndLock', 'getAndTouch', 'getMulti', 'getReplica', 'insert', 'prepend', 'query', 'upsert', 'remove', 'replace', 'unlock');
        ...
        for (let collection of this._collections) {
            ...
            if (initialize) {
                const cIndexes = await collection.updateIndexes(indexes);
                toBuild = toBuild.concat(cIndexes);
            }
        }
        if (toBuild.length) {
            const buildIndexes = couchbase_1.N1qlQuery.fromString(`
                BUILD INDEX ON \`${this.id}\`(${toBuild.join(',')}) USING GSI;
            `);
            this.query(buildIndexes.consistency(couchbase_1.N1qlQuery.Consistency.STATEMENT_PLUS));
        }
    }
    async _getIndexes() {
...
__decorate([
    queries_1.index('_type').asProp
], Bucket.prototype, "ix_collection_type", void 0);
Bucket = __decorate([
    type_r_1.define,
